...
from tools.per_core_status import PerCoreStatusTool  # noqa: E402
from tools.models import ToolOutput  # noqa: E402
from utils.per_core_tool_integration import enhance_tool_execution  # noqa: E402
...
TOOLS = {
    ...
    "per_core_status": PerCoreStatusTool(),  # Check per-core agent system status and health
}
...
async def handle_list_tools() -> list[Tool]:
    ...
    for tool in TOOLS.values():
        ...
        # Remove outputSchema to avoid validation errors
        # Tools return TextContent with JSON strings, not structured data
        output_schema = None
...
async def handle_call_tool(name: str, arguments: dict[str, Any]) -> list[TextContent]:
    ...
    if name in TOOLS:
        ...
        if not tool.requires_model():
            ...
            # Execute tool with per-core agent integration if available
            result, used_per_core = await enhance_tool_execution(tool, arguments)
            if used_per_core:
                logger.info(f"Tool '{name}' executed with per-core agent coordination")
            return result
        ...
        if used_per_core:
            ...
            logger.info(f"Tool '{name}' executed with standard single-agent mode")
...
async def initialize_per_core_agent_system():
    """
    Initialize the per-core agent coordination system during server startup.
    
    This function integrates all per-core agent components into the main MCP server
    startup sequence, performs system-wide validation, and ensures proper health checks.
    """
    try:
        from config import ENABLE_PER_CORE_AGENTS
        ...
        logger.info("Initializing per-core agent coordination system...")
        
        # Get the global integrator instance and initialize it
        integrator = get_per_core_integrator()
        
        # Initialize the per-core agent system
        ...
        if success:
            ...
            if validation_results["overall_health"]:
                logger.info("✅ Per-core agent system initialized successfully")
                logger.info(f"   - {validation_results['num_agents']} agents active")
                logger.info(f"   - {len(validation_results['agent_roles'])} unique roles assigned")
                logger.info(f"   - System health: {validation_results['health_status']}")
                
                # Log agent assignments
                if validation_results.get("agent_details"):
                    logger.debug("Agent assignments:")
                    for agent_detail in validation_results["agent_details"]:
                        logger.debug(f"   - Core {agent_detail['core_id']}: {agent_detail['role']} "
                                   f"({agent_detail['status']})")
                
                return True
            else:
                logger.warning("Per-core agent system validation failed")
                logger.warning(f"Health issues: {validation_results.get('health_issues', [])}")
                return False
        else:
            logger.info("Per-core agent system initialization failed, falling back to single-agent mode")
            return False
            
    except Exception as e:
        logger.error(f"Error initializing per-core agent system: {e}")
        logger.info("Falling back to single-agent mode")
        return False


async def validate_per_core_system(integrator) -> Dict[str, Any]:
    """
    Perform comprehensive system-wide validation and health checks for the per-core agent system.
    
    Args:
        integrator: The PerCoreToolIntegrator instance
        
    Returns:
        Dictionary containing validation results and system health information
    """
    validation_results = {
    ...
    try:
        if not integrator.per_core_manager:
            validation_results["health_issues"].append("Per-core manager not initialized")
            return validation_results
        
        # Get system health information
        system_health = integrator.per_core_manager.get_system_health()
        
        validation_results.update({
            "num_agents": system_health.get("total_agents", 0),
            "agent_roles": list(set(agent.get("role") for agent in system_health.get("agents", []))),
            "health_status": system_health.get("overall_status", "unknown")
        })
        
        # Validate individual agents
        healthy_agents = 0
        for agent_info in system_health.get("agents", []):
            agent_detail = {
                "agent_id": agent_info.get("agent_id"),
                "core_id": agent_info.get("core_id"),
                "role": agent_info.get("role"),
                "status": agent_info.get("status"),
                "healthy": agent_info.get("status") == "active"
            }
            validation_results["agent_details"].append(agent_detail)
            
            if agent_detail["healthy"]:
                healthy_agents += 1
            else:
                validation_results["health_issues"].append(
                    f"Agent {agent_info.get('agent_id')} on core {agent_info.get('core_id')} "
                    f"is {agent_info.get('status')}"
                )
        
        # Check communication system
        if integrator.communication_system:
            validation_results["system_capabilities"]["communication"] = True
        else:
            validation_results["health_issues"].append("Agent communication system not available")
            validation_results["system_capabilities"]["communication"] = False
        
        # Check OpenRouter connectivity if configured
        from config import PER_CORE_OPENROUTER_REQUIRED
        import os
        
        openrouter_key = os.getenv("OPENROUTER_API_KEY")
        if openrouter_key and openrouter_key != "your_openrouter_api_key_here":
            validation_results["system_capabilities"]["openrouter"] = True
        elif PER_CORE_OPENROUTER_REQUIRED:
            validation_results["health_issues"].append("OpenRouter API key required but not configured")
            validation_results["system_capabilities"]["openrouter"] = False
        else:
            validation_results["system_capabilities"]["openrouter"] = False
        
        # Overall health assessment
        # Check for emergency agents
        has_emergency_agent = any(
            "emergency_agent" in agent.get("agent_id", "") 
            for agent in system_health.get("agents", [])
        )
        
        # When OpenRouter is not required, we're more lenient about health
        if not PER_CORE_OPENROUTER_REQUIRED:
            # System is healthy if:
            # 1. We have any agents (healthy or emergency)
            # 2. Communication system is working
            validation_results["overall_health"] = (
                (validation_results["num_agents"] > 0 or has_emergency_agent) and
                validation_results["system_capabilities"]["communication"]
            )
        else:
            # When OpenRouter is required, we need healthy agents
            validation_results["overall_health"] = (
                healthy_agents > 0 and
                len(validation_results["health_issues"]) == 0 and
                validation_results["system_capabilities"]["communication"]
            )
        
        # If we have emergency agents, note this in the results
        if has_emergency_agent:
            validation_results["emergency_mode"] = True
            validation_results["health_status"] = "emergency_fallback"
        elif validation_results["num_agents"] > 0:
            validation_results["emergency_mode"] = False
        
        # Add performance metrics if available
        if hasattr(integrator.per_core_manager, 'get_performance_metrics'):
            try:
                metrics = integrator.per_core_manager.get_performance_metrics()
        ...
        validation_results["health_issues"].append(f"Validation error: {str(e)}")
        return validation_results
...
async def main():
    ...
    # Initialize per-core agent coordination system
    per_core_initialized = await initialize_per_core_agent_system()
    
    if per_core_initialized:
        logger.info("🤖 Multi-agent coordination active - tools will leverage specialized agents")
    else:
        logger.info("🔧 Single-agent mode active - tools will use standard execution")
