"""
Configuration and constants for Nexus MCP Server

This module centralizes all configuration settings for the Nexus MCP Server.
...
# Per-Core Agent System Configuration
# Configuration for the per-core agent coordination system that creates
# autonomous AI agents corresponding to available CPU cores

# ENABLE_PER_CORE_AGENTS: Enable/disable the per-core agent system
# When enabled, tools will automatically leverage multiple specialized agents
# for enhanced multi-perspective analysis and coordination
# When disabled, tools fall back to standard single-agent operation
ENABLE_PER_CORE_AGENTS = os.getenv("ENABLE_PER_CORE_AGENTS", "true").lower() in ("true", "1", "yes", "on")

# PER_CORE_MAX_AGENTS: Maximum number of agents to create
# If not set, uses all available CPU cores
# Useful for limiting resource usage on high-core-count systems
PER_CORE_MAX_AGENTS = None
if os.getenv("PER_CORE_MAX_AGENTS"):
    try:
        PER_CORE_MAX_AGENTS = int(os.getenv("PER_CORE_MAX_AGENTS"))
    except (ValueError, TypeError):
        PER_CORE_MAX_AGENTS = None

# PER_CORE_OPENROUTER_REQUIRED: Whether OpenRouter is required for per-core agents
# When True, per-core system only activates if OpenRouter API key is configured
# When False, per-core system can work with any available providers
PER_CORE_OPENROUTER_REQUIRED = os.getenv("PER_CORE_OPENROUTER_REQUIRED", "false").lower() in ("true", "1", "yes", "on")

# PER_CORE_FALLBACK_MODE: Behavior when per-core system is unavailable
# "single_agent": Fall back to standard single-agent execution (default)
# "error": Return error if per-core system cannot be initialized
# "disabled": Disable per-core features entirely
PER_CORE_FALLBACK_MODE = os.getenv("PER_CORE_FALLBACK_MODE", "single_agent").lower()

# PER_CORE_MULTI_AGENT_TOOLS: Tools that benefit from multiple agent perspectives
# These tools will attempt to use multiple agents when available for enhanced analysis
PER_CORE_MULTI_AGENT_TOOLS = {
    "consensus", "thinkdeep", "parallelthink", "codereview", "secaudit", "analyze"
}

# PER_CORE_HEALTH_CHECK_INTERVAL: Interval in seconds for agent health monitoring
PER_CORE_HEALTH_CHECK_INTERVAL = int(os.getenv("PER_CORE_HEALTH_CHECK_INTERVAL", "60"))

# PER_CORE_AGENT_TIMEOUT: Timeout in seconds for individual agent operations
PER_CORE_AGENT_TIMEOUT = int(os.getenv("PER_CORE_AGENT_TIMEOUT", "300"))  # 5 minutes

# Per-Core Agent Configuration Management
# The per-core agent system uses a dedicated configuration manager for secure
# API key handling, configuration validation, and hot-reload capabilities
def get_per_core_agent_config():
    """Get per-core agent configuration with validation and hot-reload support"""
    try:
        from utils.per_core_agent_config import get_per_core_agent_config_manager
        config_manager = get_per_core_agent_config_manager()
        return config_manager.get_config()
    except ImportError:
        # Fallback to basic configuration if per-core config module not available
        from dataclasses import dataclass
        from typing import Optional, Set
        
        @dataclass
        class BasicPerCoreConfig:
            enabled: bool = ENABLE_PER_CORE_AGENTS
            max_agents: Optional[int] = PER_CORE_MAX_AGENTS
            openrouter_required: bool = PER_CORE_OPENROUTER_REQUIRED
            fallback_mode: str = PER_CORE_FALLBACK_MODE
            health_check_interval: int = PER_CORE_HEALTH_CHECK_INTERVAL
            agent_timeout: int = PER_CORE_AGENT_TIMEOUT
            openrouter_api_key: Optional[str] = os.getenv('OPENROUTER_API_KEY')
            multi_agent_tools: Set[str] = PER_CORE_MULTI_AGENT_TOOLS
        
        return BasicPerCoreConfig()
    except Exception as e:
        import logging
        logging.warning(f"Failed to load per-core agent configuration: {e}")
        # Return disabled configuration on error
        from dataclasses import dataclass
        
        @dataclass
        class DisabledPerCoreConfig:
            enabled: bool = False
            fallback_mode: str = "single_agent"
        
        return DisabledPerCoreConfig()