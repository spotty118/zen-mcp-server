...
class ParallelThinkTool(BaseTool):
    ...
    def _get_available_models(self) -> list[str]:
        ...
        try:
            ...
            preferred_per_provider: dict[ProviderType, list[str]] = {
                ProviderType.OPENAI: ["o3", "o3-mini", "o3-mini-high", "o4-mini", "gpt-4.1"],
                ProviderType.GOOGLE: ["gemini-2.5-pro", "gemini-2.5-flash", "gemini-2.0-flash", "gemini-2.0-flash-lite"],
                ProviderType.XAI: ["grok-4", "grok-3", "grok-3-fast"],
                ProviderType.DIAL: [],
            ...
            for provider_type in provider_order:
                try:
                    ...
                    available = registry.get_available_model_names(provider_type)
                    if not available:
                        continue

                    candidates = preferred_per_provider.get(provider_type, [])
                    selected = None
                    ...
                    available_models.append(selected)
                except Exception as e:
                    logger.debug(f"Could not get models from {provider_type.value}: {e}")
    ...
    async def execute(self, arguments: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        try:
            ...
            # Convert to proper ToolOutput format for MCP compliance
            ...
            error_output = ToolOutput(
                status="error",
                content=f"Parallel thinking failed: {str(e)}",
                content_type="text",
